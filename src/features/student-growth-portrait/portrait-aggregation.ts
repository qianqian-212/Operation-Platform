...
export const defaultPortraitAggregationRules: PortraitAggregationRules = {
  version: "student-growth-aggregation/v2",
...
function evaluationDistribution(
  ...
  const levels = ["excellent", "average", "needs-effort"] as const;
  return {
    ...
    items: levels.map((level) => ({
      key: level,
      value: percent(records.filter((record) => record.level === level).length, records.length),
      studentCount: records.filter((record) => record.level === level).length,
    })),
  };
}
...
function buildMetrics(
  ...
  const practice = events.filter((record) => "activityId" in record && record.isVerified);
...
export function aggregatePortraitDataset(
  ...
  const schools: SchoolPortraitSummary[] = [...new Set(students.map((student) => student.schoolId))].map((schoolId) => {
    ...
    return {
      ...
      metrics: schoolStudents.length >= rules.minimumPublishableGroupSize
        ? buildMetrics(events.filter((event) => event.schoolId === schoolId), schoolPopulation).metrics
        : [],
    };
  });
