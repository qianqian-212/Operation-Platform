...
test("智慧体育数据驾驶舱使用独立业务壳并复用共享地图基础", async ({
  ...
  await expect(sportsPage.getByRole("button", { name: "切换至生态荧光" }))
    .toBeVisible();
  const sportsDateRange = sportsPage.getByRole("group", { name: "智慧体育统计时间" });
  await expect(sportsDateRange).toBeVisible();
  await expect(sportsDateRange.locator("input")).toHaveCount(2);
  await expect(sportsDateRange.locator("input").first()).not.toHaveValue("");
  await expect(sportsDateRange.locator("input").last()).not.toHaveValue("");
  const initialOverviewCoverage = await sportsPage
    .getByLabel("智慧体育范围概览")
    .locator(".sports-summary__primary dd")
    .first()
    .textContent();
  await sportsDateRange.locator("input").first().click();
  await expect(sportsPage.getByRole("button", { name: "最近一周", exact: true })).toBeVisible();
  await expect(sportsPage.getByRole("button", { name: "最近一个月", exact: true })).toBeVisible();
  await expect(sportsPage.getByRole("button", { name: "最近三个月", exact: true })).toBeVisible();
  await sportsPage.getByRole("button", { name: "最近一周", exact: true }).click();
  await expect(sportsPage.getByLabel("智慧体育范围概览").locator(
    ".sports-summary__primary dd",
  ).first()).not.toHaveText(initialOverviewCoverage ?? "");
  ...
  const mapCanvas = sportsPage.locator(".regional-map-canvas");
  await expect.poll(async () => Number(await mapCanvas.getAttribute("data-render-calls")))
    .toBeGreaterThan(0);
  const initialMapResources = await mapCanvas.evaluate((element) => {
  ...
  const runningCards = sportsPage.getByLabel("广东省阳光长跑运动概览");
  await expect(runningCards.locator("article")).toHaveCount(3);
  await expect(runningCards.getByText("有氧运动强度", { exact: true })).toBeVisible();
  await expect(sportsPage.getByText("阳光长跑 TOP5", { exact: true })).toBeVisible();
  await expect(sportsPage.locator(".top-five-select")).toHaveCount(0);
  await expect(sportsPage.getByRole("tab", { name: "累计运动路程", exact: true }))
    .toHaveAttribute("aria-selected", "true");
  await expect(sportsPage.getByLabel("地市累计运动路程排行榜")).toContainText("万km");
  await expect(sportsPage.getByLabel("阳光长跑累计路程排行榜")).toContainText("万km");
  await expect(runningCards.locator(".sports-intensity-card")).toHaveClass(/sports-goal-card/);
  await expect(runningCards.locator(".intensity-progress").first().locator("i")).toHaveCount(10);
  const inactiveIntensitySegment = runningCards
    .locator(".intensity-progress")
    .first()
    .locator("i")
    .last();
  await expect(inactiveIntensitySegment).toHaveCSS("height", "6px");
  await expect(inactiveIntensitySegment).toHaveCSS(
    "background-color",
    "rgba(255, 255, 255, 0.24)",
  );
  for (const value of [
    sportsPage.getByLabel("智慧体育范围概览").locator(".sports-summary__primary dd").first(),
    runningCards.locator(".sports-intensity-card dd").first(),
  ]) {
    await expect(value).toHaveCSS("font-size", "28px");
    await expect(value.locator("small")).toHaveCSS("font-size", "14px");
  }
  await expect(runningCards.locator(".sports-intensity-card dd").first())
    .toHaveCSS("font-weight", "400");
  await expect(runningCards.locator(".sports-intensity-card dd").nth(1))
    .toHaveCSS("font-weight", "400");
  await expect(runningCards.getByText("本周运动目标完成情况", { exact: true })).toBeVisible();
  await expect(runningCards.getByRole("tab", { name: "运动人数", exact: true })).toBeVisible();
  await expect(runningCards.getByRole("tab", { name: "运动次数", exact: true })).toBeVisible();
  await expect(runningCards.getByRole("tab", { name: "日参与率", exact: true })).toBeVisible();
  const runningSummaryBounds = (await sportsPage
    .getByLabel("智慧体育范围概览")
    .boundingBox())!;
  const runningMapControlsBounds = (await sportsPage
    .getByLabel("地图视角与点位控制")
    .boundingBox())!;
  expect(runningSummaryBounds.y + runningSummaryBounds.height)
    .toBeLessThanOrEqual(runningMapControlsBounds.y - 8);
  await sportsPage.getByRole("tab", { name: "AI体测", exact: true }).click();
  const assessmentSummary = sportsPage.getByLabel("智慧体育范围概览");
  await expect(assessmentSummary).toContainText("省级参与体测人数");
  await expect(assessmentSummary).toContainText("体测总人数");
  await expect(assessmentSummary).toContainText("测试项目数");
  await expect(assessmentSummary).toContainText("省级平均分");
  await expect(assessmentSummary).toContainText("国标达标率");
  await expect(assessmentSummary.locator(".sports-summary__metrics li")).toHaveCount(2);
  const assessmentCards = sportsPage.getByLabel("广东省AI体测概览");
  await expect(assessmentCards.getByText("体测等级分布", { exact: true })).toBeVisible();
  await expect(assessmentCards.getByText("优秀", { exact: true })).toBeVisible();
  await expect(assessmentCards.getByText("良好", { exact: true })).toBeVisible();
  await expect(assessmentCards.getByText("及格", { exact: true })).toBeVisible();
  await expect(assessmentCards.getByText("不及格", { exact: true })).toBeVisible();
  await expect(assessmentCards.locator(".sports-grade-card .sports-summary__primary dd small")).toHaveText([
    "人",
    "人",
    "人",
    "人",
  ]);
  await expect(assessmentCards.getByText("本学期体测完成率", { exact: true })).toBeVisible();
  await expect(assessmentCards.getByText("体测项目达标率", { exact: true })).toBeVisible();
  await expect(assessmentCards.getByRole("img", {
    name: "广东省体测项目达标率雷达图",
  })).toBeVisible();
  await expect(sportsPage.getByText("AI体测 TOP5", { exact: true })).toBeVisible();
  await expect(sportsPage.getByLabel("AI体测合格率排行榜")).toContainText("92.6%");
  await sportsPage.getByRole("tab", { name: "阳光长跑", exact: true }).click();
  const provinceRunningCoverage = await sportsPage
    .getByLabel("智慧体育范围概览")
    .locator(".sports-summary__primary dd")
    .first()
    .textContent();
  ...
  await expect.poll(async () => Number(await canvas.getAttribute("data-render-calls")))
    .toBeGreaterThan(0);
  const provinceRenderBudget = await canvas.evaluate((element) => {
    const dataset = (element as unknown as {
      dataset: Record<string, string | undefined>;
    }).dataset;
    return {
  ...
  expect(provinceRenderBudget.calls).toBeLessThan(260);
  expect(provinceRenderBudget.geometries).toBeLessThan(180);
  expect(provinceRenderBudget.calls).toBe(initialMapResources.calls);
  expect(provinceRenderBudget.geometries).toBe(initialMapResources.geometries);
  ...
  const citySummary = sportsPage.getByLabel("智慧体育范围概览");
  await expect(citySummary).toContainText("市级覆盖人数");
  await expect(citySummary.locator(".sports-summary__primary dd").first())
    .not.toHaveText(provinceRunningCoverage ?? "");
  await expect(sportsPage.getByText(`${selectedCityName}指标排名`, { exact: true })).toBeVisible();
  await expect(sportsPage.getByRole("button", { name: "区县对比", exact: true })).toBeVisible();
  await expect(sportsPage.getByLabel(`${selectedCityName}阳光长跑运动概览`)).toBeVisible();
  ...
  expect(renderBudget.calls).toBeLessThan(500);
  ...
  expect(renderBudget.geometries).toBeLessThan(400);
...
test("区域教育总览从教育局菜单打开独立数字孪生首页", async ({ page }) => {
  ...
  await expect(overviewPage.getByRole("group", { name: "智慧体育统计时间" }))
    .toHaveCount(0);
