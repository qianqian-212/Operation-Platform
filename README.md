...
- 教育局“新学生成长画像”SaaS 页面：以八个同级锚点连续展示区域总览、五育评价、运动健康、荣誉发展、行为习惯、实践活动、日常评价和区域数据覆盖。六类综合素质模块复用学生成长画像数据契约、指标字典和聚合仓库，只发布可追溯的区域聚合结果，不生成综合总分、个人名单、学生群体标签或学校发展排名。区域总览只保留学生规模、单科平均得分率和可直接核验的记录覆盖率；指标卡标题并入口径说明，底部展示同比变化（对照上一学年同一学期，百分点差以 % 展示），上涨与下跌使用不同颜色区分。不发布相邻学期环比，以避免上、下学期活动与考试安排不可比。统考平均得分率与覆盖率在总览内通过年级、科目口径选择器限定同一考试计划，页面筛选为“全部年级”时自动选中当前科目下首个可比较年级，避免默认显示为“—”。当前没有城乡、片区和学校类型主数据，因此页面不提供学校类型筛选、分类差异或区域均衡指数，改为展示各来源的有效学生数、在籍分母、覆盖率与计算方式。
