...
- 教育局“新学生成长画像”SaaS 页面：以八个同级锚点连续展示区域总览、五育评价、运动健康、荣誉发展、行为习惯、实践活动、日常评价和区域数据覆盖。六类综合素质模块复用学生成长画像数据契约、指标字典和聚合仓库，只发布可追溯的区域、年级与受限学校聚合结果，不生成综合总分、个人名单、学生群体标签或学校发展排名。页面筛选在查询成功后形成统一的已应用范围；除区域学生总数始终使用区域有效学籍总量外，所有内容模块统一按已应用的学段、年级和学期查询，学业模块再叠加卡片内科目条件。指标卡底部展示上一学年同学期的同比变化：百分率使用百分点，数量和人均量使用相对增幅，并以颜色区分上升、下降与不可比；缺少历史期或质量不足时显示暂无可比，不把缺失值当作零。不发布相邻学期环比，以避免上、下学期活动与考试安排不可比。统考平均得分率与成绩覆盖率从通用总览中独立为年级组合图；期中、期末阶段对比同样以年级为横轴，每个年级并列展示两根柱，不跨年级或试卷加权合并。五育评价不假定学校使用固定评价等级，只展示有效评价记录覆盖率。运动健康从原始目标、体测和跑步记录聚合学生数、体育目标完成率、分学段体测达标率及阳光长跑累计量和人均量；体测与长跑采用同级数据组展示，未接入医疗模型时不生成健康风险结论。荣誉发展从明确的奖项类型、奖项级别和奖项等级字段聚合数量，不依据奖项名称猜测类别。综合素质模块默认按年级比较，仅对口径稳定且能明确说明边界的覆盖或参与指标开放学校对比。当前没有城乡、片区和学校类型主数据，因此页面不提供学校类型筛选、分类差异或区域均衡指数，改为展示各来源的有效学生数、在籍分母、覆盖率与计算方式。
...
| `src/features/new-student-growth-portrait/` | 新学生成长画像的锚点配置、区域综合素质模块、演示图表配置与局部图表运行组件 |
...
区域学生发展画像当前通过 `virtual-portrait-raw-data-source.ts` 提供明确标记的虚拟学生原始记录，并由 `data-contract.ts`、`student-growth-portrait-repository.ts` 与 `portrait-aggregation.ts` 聚合为 `PortraitDataset`。`metric-registry.ts` 是指标字典，`page-capability-matrix.ts` 约束数据领域与内容组件；新学生成长画像的八个同级锚点由 `src/features/new-student-growth-portrait/data.ts` 维护，锚点不是路由或独立页面。统考原始记录必须带考试类型、组织范围、考试计划、试卷版本、年级、学科、成绩、满分、排名与参考人数；页面当前只发布同批次区域平均得分率、描述性得分率变化及数据覆盖事实。知识模块、正式增值、三期以上稳定性、学校发展、同类学校、城乡和片区分析必须等相应来源、主数据与模型就绪后再进入页面。接入真实后端时只替换原始数据源适配器；指标必须携带分子、分母、覆盖率、规则版本和可比性，不得把展示常量当作可追溯结论。心理健康、医疗与消费数据需独立授权、脱敏和审计，不参与综合评分或公开排名。
