# 项目协作规范

本文件适用于整个仓库。项目是 Vue 3 + TypeScript 多租户教育 SaaS 前端，生产数据默认由 Supabase Auth、Postgres 和 RLS 管理。

## 开始前

- 先阅读 `README.md`，再以当前代码和测试为事实源。
- 保留与任务无关的工作区修改；禁止用破坏性 Git 命令覆盖用户文件。
- 优先复用现有配置、Store、repository、组件和设计变量，不在视图中复制领域规则。

## 架构边界

- `src/config/page-registry.ts` 是页面资源和动态路由的唯一代码级事实源。菜单只保存 `pageKey`，不得保存组件路径。
- 菜单模板只用于首次初始化和恢复默认，不得静默覆盖租户现有配置。菜单最多四级，层级规则由领域校验统一维护。
- 工作台是租户 Shell 配置，不是菜单节点；固定组件目录以 `src/features/workbench/workbench-widget-catalog.ts` 为准，各组织类型的默认编排以 `src/features/workbench/workbench-templates.ts` 为准。系统组件集 = 目录 ∩ 当前授权，授权只能在组件声明的兼容组织类型内启用或停用。公共组件只定义一次，领域组件通过兼容组织类型引用，个人布局不得真正新增或删除系统组件。数据概览覆盖全部组织类型，按类型与角色切换指标；消息与待办中心面向学校、教育局和机构，不含运营平台。
- `src/features/persistence/operation-platform-persistence.ts` 是应用持久化契约，`runtime-operation-platform-persistence.ts` 是唯一装配入口。页面和 Store 不得直接依赖 Adapter、远端 DTO 或数据源类型。
- 菜单、工作台入口和角色必须通过 `src/features/tenant-config/` 聚合原子保存；Supabase 写入保留 revision 并发控制。
- 组织、成员、工作台和权限分别通过对应 `src/features/` repository 维护。Repository 返回防御性副本、显式抛错；损坏数据只有在备份成功后才能恢复。
- 当前机构只保存在按用户隔离的登录会话中，退出登录时清除；激活角色、主题和个人布局统一经过持久化契约。

## 权限与数据安全

- RBAC 默认拒绝。导航、默认入口、工作台和路由守卫必须使用同一当前租户与激活角色语义。
- 当前用户角色来自启用的组织成员记录；禁用成员或角色不参与权限计算。不得删除、禁用或移除最后一个启用管理员。
- 管理员拥有当前租户全部可见叶子菜单；普通角色只授权叶子节点，父级按获权子节点保留。
- 浏览器只能使用 Supabase publishable key。数据库密码、Secret Key、迁移账号和模型密钥不得进入 `VITE_*`。
- 所有浏览器可访问表必须启用 RLS，并遵循用户身份、租户归属和最小权限。前端权限不是生产安全边界。

## 导航与界面

- `AppLayout.vue` 是持续挂载的应用 Shell；Vue Router 是页面与导航高亮的事实源，导航前不要清空状态。
- 内部导航使用 Router API；只有外部链接使用 `<a>` 或 `window.open`，并遵循配置的打开方式。
- 使用 `src/styles` 设计变量和现有 Element Plus 组件，避免可复用颜色、间距和尺寸魔法值。
- 不增加无需求依据的路由动画；可见交互应保持键盘、焦点和响应式行为。

## 区域教育数字孪生

- `src/features/digital-twin/` 是两套驾驶舱唯一的共享基础层，维护地图引擎、行政区状态机、Provider、主题、设计变量、动效、通用图表、页面壳、地图工作区和会话能力；该目录的生产代码不得依赖 `regional-education-overview` 或 `smart-sports-dashboard`。主题和 `charts--*` 变量只作用于 `.regional-digital-twin`，不得污染 SaaS Shell。
- `src/features/regional-education-overview/` 与 `src/features/smart-sports-dashboard/` 是两个独立业务边界，只能单向依赖 `digital-twin`，彼此不得引用。区域业务独占区域菜单、状态栏和学业分析；智慧体育独占体育 HUD、模拟数据、覆盖人数和体育地图适配器。地图根范围以当前组织的 `administrativeRegion` 为唯一事实源；广东省地市首屏仅在组织选择广东省时使用静态内置优化，其余边界按当前分支懒加载并缓存。
- 组织配置的省、市或区县必须是地图导航的最高层级，面包屑、返回、同级切换和外部轮廓交互均不得越出该根范围。根范围内的父子层交互保持统一：进入下级后复用父级同级层，在当前范围内叠加子级活动层；聚焦区县后，区县同级保持厚度，地市层切换为外部轮廓。智慧体育默认在区县终止；区域教育仅在存在可用 Provider 时继续到镇街。榕城区教育局租户 `bureau-74b5bcaf-69af-4cf2-8c63-11f9270d4676` 的区域教育与智慧体育均通过高优先级本地 Provider 使用现行 16 街镇边界并允许镇街聚焦；特殊性只存在于数据覆盖和终止层级，不得派生独立导航逻辑。
- 区域教育与智慧体育必须共用同一地图引擎和导航实现。下钻、同级切换、返回、面包屑和自动巡航必须经过同一可取消导航事务；用户事务可抢占边界加载、图层预构建、相机过渡和自动巡航，自动事务不得阻塞或覆盖用户操作。学校网格只在区县及以下且存在点位数据时启用，学校搜索控件始终保留；能量锥峰的指标与数值由业务数据源注入，不得派生地图实现分支。地图返回过渡必须先响应相机，再准备学校网格等业务图层。
- 所有行政区层级及外部轮廓的名称默认隐藏，只在指针移入对应区域时显示；活动层、同级层和外部层必须使用同一标签交互语义。
- 行政区导航必须由通用可变深度状态机和边界 Provider 链决定分支终点，不得在地图引擎或页面中为榕城区、省、市、区、镇分别维护下钻流程。业务数据源通过 Provider 可用性决定最深层级，本地高优先级 Provider 可以补充公开数据标记为叶子的分支。
- 几何层级变化时应将数据请求、相机聚焦和图层预构建并行化；稳定的父级同级几何必须转移所有权并直接复用，不得重复构建。提交阶段只安装已准备图层，不得通过整个地图 Group 的 Z 位移或缩放模拟下钻；投影不变时不得重置相机。预构建取消、减少动态效果和页面卸载都必须释放任务自有资源，不得销毁已转移的复用层。
- 同一几何带内聚焦终端子级时，父级锥峰、学校点位和连接网络必须暂停并保留所有权；返回父级时直接恢复原业务层，不得在状态提交路径同步重建。缓存只保留当前导航分支的一个父级，切换数据模式、点位集合、几何分支或卸载页面时必须释放。
- 所有父子层级使用同一空间交互语义：预览阶段抬升被选分支，非终点活动子层整组沿用终点聚焦参数，稳定活动几何跨状态转移复用；同级厚度层用于同级切换，外部平面轮廓和嵌套层空白点击用于返回上一级。面包屑必须基于完整 `navigationPath` 展示并支持返回任一祖先层级。
- 抽取数字孪生共享能力时以区域教育和智慧体育的视觉回归为门禁，优先抽地图引擎、导航、主题、设计变量、Dashboard 容器和动效，不得在抽取过程中改变现有 DOM 构图、响应式尺寸或主题结果。
- 行政区位于 `XY` 平面，`Z` 表示厚度和高度；相机及 OrbitControls 必须使用 `Z-up`。
- 地图渲染链路统一使用 GCJ-02；DataV 与已配准的 GCJ-02 数据直接进入地图，WGS84 点位或几何只能在数据源入口转换一次，视图、相机和渲染层不得各自补偿偏移。
- 保持左键受限旋转、滚轮缩放、关闭平移、阻尼和俯仰限制。重置视角必须平滑过渡。
- 区县与镇街下钻复用同一 Scene 和投影，通过相机与图层状态过渡；不得销毁重建场景模拟下钻。过渡前清理残余阻尼，并保持 HUD 构图偏移稳定。
- 手动操作暂停自动平面旋转，10 秒无操作后从当前视角连续恢复。行政区子级巡航、学校轮播和锥峰轮播是独立机制，不能相互等待或破坏自动旋转。
- Three.js Geometry、Material、Texture 和 CSS2D 标签必须有明确所有权与销毁路径。隐藏页面不得继续渲染；高频更新应合并到动画帧并遵守空闲/交互帧率策略。
- 图表通过通用 Dashboard 面板和 `EChartCanvas` 扩展；保持模块化注册、ResizeObserver 合帧和卸载销毁。

## 菜单配置

- 行内编辑使用 `Enter` 保存、`Escape` 取消；抽屉入口继续保留。
- 筛选时禁用拖拽；拖拽、编辑和保存后不得自动展开节点。
- 页面选项来自页面注册表。图标使用现有 Lucide 动态加载，不整体导入图标库。

## 验证与交付

```bash
npm run check
npm run test:e2e
npm audit --offline
```

- 提交前至少运行 `npm run check`。
- 修改路由、菜单、权限或 repository 时补充单元测试；修改可见交互时运行相关 Playwright 用例。
- 浏览器自动化优先使用 Playwright Test；临时脚本必须在 `finally` 中关闭 page、context、browser 和自启服务，结束后按本任务记录的 PID 与端口核查，不得遗留 daemon、无头浏览器或 Vite 进程。
- 不把单元测试描述为端到端验证，不把局部检查描述为完整门禁。
- 提交按单一职责拆分，不混入无关格式化；合并主分支前保持工作区干净。
- 架构边界、命令、入口或存储约定变化时同步更新 `README.md`；协作约束变化时更新本文件。
